...
<!doctype html>
    ...
    <title>Comunicação Interna - Todas as comunicações.</title>
        ...
        <!-- <div class="container">
            <div class="row">
              <div class="col bg text-left pt-4" >
                <h4>Pesquisa de comunicações por chapa e periodo. Chapa: <?php print $txt_chapa; ?> <span class="badge badge-warning"><?php print strtolower($login) . " | " . mysql_result($getUserDept,0); ?></span></h4>
              </div>
            </div>
        </div> -->
        ...
        if (!$txt_data_ini || !$txt_data_fim) {

            print "Data inválida. Clique <a href='javascript:history.go(-1);' > AQUI </a> para voltar.";
            exit();

        }

        if ($_POST["proc_ro"]) {
          if ($_POST["txt_tipo"] == "ro") { 
                $chave  = "REL_OPERACAO_ID";

                if (count($_POST['slc_ro']) == 0) {
                    print "Escolha uma ocorrencia. <a href='javascript:history.go(-1);'>Voltar</a>";
                    exit();
                }

                foreach($_POST['slc_ro'] as $itens) {
                    $itens2 .= $itens . ",";
                }
                $itens2 = rtrim($itens2,',');
          }
        ...
        if ($_POST["proc_cn"]) {
          if ($_POST["txt_tipo"] == "cn") {
                $chave = "CN_ID";
                
                if (count($_POST['slc_cn']) == 0) {
                    print "Escolha uma ocorrencia. <a href='javascript:history.go(-1);'>Voltar</a>";
                    exit();
                }

                foreach($_POST['slc_cn'] as $itens) {
                    $itens2 .= $itens . ",";
                }
                $itens2 = rtrim($itens2,',');
          }
        ...
        if($_POST["proc_cv"]) {
            if ($_POST["txt_tipo"] == "cv") {
                $chave = "CUIDVEIC_ID";

                if (count($_POST['slc_cv']) == 0) {
                    print "<b>Escolha uma ocorrencia.</b> <a href='javascript:history.go(-1);'>Voltar</a>";
                    exit();
                }
